...
export const expandCompares = (
  compareCategory: string,
  queries: Map<string, Array<string>>
): Map<string, Array<Query>> => {
  const out: Map<string, Array<Query>> = {}
  const compareDimensions = queries[compareCategory]
  if (compareDimensions === undefined) {
    throw new Error(
      `Cannot compare category that does not exist in queries. category: ${compareCategory} queries: ${queries}`
    )
  }
