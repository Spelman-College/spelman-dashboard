...
describe('set intersection returns the correct values', () => {
	...
	expect(setIntersection(new Set(['cat']),
			       new Set(['cat', 'dog']))).toEqual(
				   new Set(['cat']))
	// Reverse argument order
	expect(setIntersection(new Set(['cat', 'dog']),
			       new Set(['cat']))).toEqual(
				   new Set(['cat']))

	expect(setIntersection(new Set(['cat', 'dog']),
			       new Set(['cat', 'dog', 'bird']))).toEqual(
				   new Set(['cat', 'dog']))
	// Reverse argument order
	expect(setIntersection(new Set(['cat', 'dog', 'bird']),
			       new Set(['cat', 'dog']))).toEqual(
				   new Set(['cat', 'dog']))
...
describe('set difference returns values in A and NOT in B', () => {
	...
	expect(setDifference(A, B)).toEqual(
	    new Set(['fish']))
    })
	...
	const C = new Set()
	expect(setDifference(C, A)).toEqual(
	    new Set())
